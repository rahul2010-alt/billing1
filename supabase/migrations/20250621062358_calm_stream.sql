/*
  ...
  3. Triggers
    - Trigger on purchase_items table to update stock on INSERT (if table exists)
    - Trigger on invoice_items table to update stock on INSERT (if table exists)
...
-- Safely handle triggers only if the target tables exist
DO $$
BEGIN
  -- Handle purchase_items table triggers
  ...
  -- Handle invoice_items table triggers  
